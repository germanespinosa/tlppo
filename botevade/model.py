...
class Model:
    ...
    def __init__(self,
        ...
        self.display = cellworld.Display(world=self.world,
                                         animated=True,
                                         fig_size=(6, 6))
        self.thread = None
        self.running = False
        self.interval = 1 / freq
        self.arena_polygon = cellworld.Polygon(self.world.implementation.space.center, 6,
                                               self.world.implementation.space.transformation.size / 2,
                                               self.world.implementation.space.transformation.rotation)
        occlusion_locations = cellworld.Location_list(iterable=[c.location for c in self.world.cells.occluded_cells()])
        self.occlusions_polygons = cellworld.Polygon_list.get_polygons(centers=occlusion_locations,
                                                                       sides=6,
                                                                       radius=self.world.implementation.cell_transformation.size / 2 * 1.05,
                                                                       rotation=self.world.implementation.cell_transformation.rotation)
        self.visibility = cellworld.Location_visibility(occlusions=self.occlusions_polygons)

    def is_valid_location(self,
                          plocation: cellworld.Location):
    ...
    def __move_agent__(self, agent_name):
        ...
        agent.theta = cellworld.normalize(agent.theta + agent.turning_speed * self.interval)
        new_location = cellworld.Location(agent.location.x, agent.location.y)
        new_location.move(theta=agent.theta,
                          dist=agent.speed * self.interval)
    ...
    def __process__(self):
        t = cellworld.Timer(self.interval)
        while self.running:
            t.reset()
            self.step()
            if self.real_time:
    ...
    def __create_observation__(self,
                               agent_name: str) -> dict:
        observation = dict()
    ...
    def show(self):
        for agent_name in self.agents_data:
            ...
            self.display.agent(agent_name=agent_name,
                               ...
                               rotation=cellworld.to_degrees(agent_data.theta),
    ...
    def add_agent(self,
        ...
        self.display.set_agent_marker(pagent_name, cellworld.Agent_markers.arrow())
