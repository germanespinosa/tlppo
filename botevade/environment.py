...
class Environment:
    def __init__(self,
        ...
        self.model.add_agent("prey",
                             prey_agent,
                             cellworld.Location(0, 0),
                             0,
                             "b",
                             pauto_update=prey_agent is not None)
        ...
        if has_predator:
            ...
            self.spawn_locations = cellworld.Location_list()
            ...
            self.model.add_agent("predator", self.predator, cellworld.Location(0, 0), 0, "r")
            self.predator_destination = self.model.display.circle(location=cellworld.Location(0, 0),
                                                                  color="b",
                                                                  alpha=.5,
                                                                  radius=0.01)
            self.predator_destination_cell = self.model.display.circle(location=cellworld.Location(0, 0),
                                                                       color="g",
                                                                       alpha=.5,
                                                                       radius=0.02)
            self.predator_capture_area = self.model.display.circle(location=cellworld.Location(0, 0),
                                                                   color="r",
                                                                   alpha=.5,
                                                                   radius=self.capture_threshold)

    def is_goal_reached(self, prey_location: cellworld.Location):
        return prey_location.dist(self.goal_location) <= self.goal_threshold

    def is_prey_captured(self, prey_location: cellworld.Location, predator_location: cellworld.Location):
    ...
    def get_observation(self) -> tuple:
        ...
        occlusions = cellworld.Location_list([(c.location.dist(prey.location), c.location) for c in self.world.cells if c.occluded])
        occlusions.sort(key=lambda a: a[0])

        def dif_dir(a: float, b: float) -> float:
            diff, direc = cellworld.angle_difference(a, b)
            return diff * direc

        max_number_of_occlusions_in_observation = 3
        closest_occlusions = [(c[0], dif_dir(prey.location.atan(c[1]), prey.theta))
                              for c in occlusions[:min(len(occlusions), max_number_of_occlusions_in_observation)]]
    ...
    def start(self) -> None:
        self.complete = False
        self.model.set_agent_position("prey",
                                      self.start_location, math.pi / 2)
        if self.has_predator:
            predator_location = random.choice(self.spawn_locations)
            predator_theta = math.pi * 2 * random.random()
            self.model.set_agent_position("predator",
                                          predator_location,
                                          predator_theta)
        self.model.start()
    ...
    def show(self) -> None:
        if self.predator:
            if self.predator.destination:
                self.predator_destination.set(center=(self.predator.destination.x,
                                                      self.predator.destination.y),
                                              color="b")
            if self.predator.destination_cell:
                self.predator_destination_cell.set(center=(self.predator.destination_cell.location.x,
                                                           self.predator.destination_cell.location.y),
                                                   color="g")
            self.predator_capture_area.set(center=(self.model.agents_data["predator"].location.x,
                                                   self.model.agents_data["predator"].location.y),
                                           color="r")
        self.model.show()
